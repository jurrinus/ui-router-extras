                ...
                $provide.decorator("$state", ['$delegate', '$log', '$q', function ($state, $log, $q) {
                    ...
                    $state.transitionTo = function (to, toParams, options) {
                        ...
                        return transitionPromise.then(function transitionSuccess(state) {
                            ...
                            if ($state.current.resolveOnEntry && $state.current.resolveOnEntry.length > 0) {
                                $state.current.resolveOnEntry.forEach(function (fct) {
                                    if ($state[fct.state] && $state[fct.state][fct.entry]) {
                                        $state[fct.state][fct.entry](fct.params ? fct.params : {});
                                    }
                                })
                            }
            ...
            $provide.decorator("$state", ['$delegate', '$rootScope', '$q', '$injector',
                function ($state, $rootScope, $q, $injector) {
                    ...
                    $rootScope.$on("$stateChangeStart", function (evt, toState, toParams, fromState, fromParams) {
                            // Need to sample this for karma testing when sendiong a boradcast.
